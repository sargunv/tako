...
const TestSession = struct {
    ...
    fn init(cols: u16, rows: u16) !TestSession {
        const resolved_font = resolveFontPath(null, null) orelse return error.FontUnavailable;
        defer allocator.free(resolved_font);

        const surface_options = SurfaceOptions{
            .font_path = resolved_font.ptr,
            .pixel_height = 14,
            .dpr = 1.0,
        };
        const surface = fontCoreCreateWithOptions(&surface_options) orelse return error.FontCoreUnavailable;
        errdefer fontCoreDestroy(surface);
        ...
        const selection_press = newSelectionEvent(@intCast(ghostty.GHOSTTY_SELECTION_GESTURE_EVENT_TYPE_PRESS)) orelse return error.SelectionEventUnavailable;
        errdefer ghostty.ghostty_selection_gesture_event_free(selection_press);
        const selection_drag = newSelectionEvent(@intCast(ghostty.GHOSTTY_SELECTION_GESTURE_EVENT_TYPE_DRAG)) orelse return error.SelectionEventUnavailable;
        errdefer ghostty.ghostty_selection_gesture_event_free(selection_drag);
        const selection_release = newSelectionEvent(@intCast(ghostty.GHOSTTY_SELECTION_GESTURE_EVENT_TYPE_RELEASE)) orelse return error.SelectionEventUnavailable;
        errdefer ghostty.ghostty_selection_gesture_event_free(selection_release);
        const selection_autoscroll_tick = newSelectionEvent(@intCast(ghostty.GHOSTTY_SELECTION_GESTURE_EVENT_TYPE_AUTOSCROLL_TICK)) orelse return error.SelectionEventUnavailable;
    ...
    fn deinit(self: *TestSession) void {
        tako_terminal_session_destroy(self.session);
    }
    ...
    fn tick(self: *TestSession) FramePlan {
        ...
        _ = tako_terminal_session_tick(self.session, &plan);
        return plan;
    }
};

fn expectSelectionText(session: ?*TerminalSession, expected: []const u8) !void {
    var buf: [256]u8 = undefined;
    const len = tako_terminal_session_selection_text(session, &buf, buf.len);
...
test "Zig core builds frames for text, default colors, cursor, and atlas" {
    ...
    try std.testing.expectEqual(@as(i32, 1), tako_terminal_session_set_default_color(ts.session, 1, true, 1, 2, 3));
    ts.write("hello");
    const plan = ts.tick();
...
test "Zig core exposes title, cwd, bell, and hyperlinks" {
    ...
    var buf: [256]u8 = undefined;
    var len = tako_terminal_session_take_title(ts.session, &buf, buf.len);
    try std.testing.expectEqualStrings("Tako Test", buf[0..len]);
    len = tako_terminal_session_take_pwd(ts.session, &buf, buf.len);
    try std.testing.expect(std.mem.endsWith(u8, buf[0..len], "/tmp"));
    try std.testing.expectEqual(@as(u32, 1), tako_terminal_session_take_bell_count(ts.session));

    ts.write("\r\n\x1b]8;;https://example.test\x07link\x1b]8;;\x07");
    const plan = ts.tick();
    len = tako_terminal_session_hyperlink_at(ts.session, plan.cell_w * 0.5, plan.cell_h * 1.5, &buf, buf.len);
...
test "Zig core selection formatting and clearing work" {
    ...
    try std.testing.expectEqual(@as(i32, 1), tako_terminal_session_selection_all(ts.session));
    try expectSelectionText(ts.session, "alpha beta");
    _ = ts.tick();

    tako_terminal_session_selection_clear(ts.session);
...
test "Zig core scrollback viewport reports state and replans" {
    ...
    try std.testing.expect(tako_terminal_session_scrollbar_state(ts.session, &state));
    try std.testing.expect(state.total >= state.len);

    tako_terminal_session_scroll_to_top(ts.session);
    const top_plan = ts.tick();
    try std.testing.expectEqual(@as(u32, 20), top_plan.cols);

    tako_terminal_session_scroll_to_bottom(ts.session);
...
test "Zig core gates frames during synchronized output" {
    ...
    var plan: FramePlan = std.mem.zeroes(FramePlan);
    try std.testing.expect(!tako_terminal_session_tick(ts.session, &plan));
